                            ...
                            <tr class="text-center odd:bg-gray-200">
                                <td class="px-4 py-2 break-words max-w-xs text-xs">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <span>{{ $order->id }}</span>
                                        <a href="{{ route('operator.order.small-edit', $order->id) }}" title="Не повне редагування" onclick="return isOperator({{ $order->operator_id ?? null }})">
                                ...
                                <td class="py-4 break-words max-w-xs text-left text-xs">
                                    <p>
                                        ...
                                        <a href="{{ route('operator.order.showUserOrders', $order->id) }}">{{ $order->user_name . ' ' . $order->user_last_name }}</a>
                                    </p>
                                    <p>
                                        ...
                                        {{ $order->delivery->region .' область, '. ($order->delivery->settlementType == 'місто' ? $order->delivery->settlementType .' '. $order->delivery->settlement : $order->delivery->district .', '. $order->delivery->settlementType .' '. $order->delivery->settlement) }}
                                    </p>
                                    <p>
                                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="12" height="12" viewBox="0 0 16 16" class="inline-block" style="fill: black">
                                        ...
                                        @elseif($order->delivery->delivery_method == 'courier')
                                            {{ (collect(['вул', 'проїзд', 'пров.', 'с.'])->first(function ($type) use ($order) { return stripos($order->delivery->street, $type) !== false; }) ? $order->delivery->street : 'вул. ' . $order->delivery->street) . ', буд. ' . $order->delivery->house . ($order->delivery->flat ? ', кв. ' . $order->delivery->flat : '') }}
                                        ...
                                        <div class="mb-4 text-xs items-center">
                                            ...
                                            <div class="flex">
                                                ...
                                                <div class="ml-4">
                                                    ...
                                                    <p class="text-gray-500 text-xs">Ціна: {{ $orderDetail->product->price->retail }}грн x1</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <td class="break-words max-w-xs text-center">
                                    <a href="{{ route('operator.order.editFirst', $order->id) }}" title="Повне редагування" class="p-2" onclick="return isOperator({{ $order->operator_id ?? null }})">
                                    ...
                                    @if($order->int_doc_number)
                                        @if($order->delivery->delivery_name == 'NovaPoshta')
                                            <a href="{{ $order->delivery->delivery_name == "NovaPoshta" ? route('operator.order.novaPoshta.ttnPdf', $order->id) : ($order->delivery->delivery_name == "UkrPoshta" ? route('operator.order.ukrPoshta.ttnPdf', $order->id) : '#') }}" target="_blank" class="p-2">
                                        ...
                                        @endif
                                    @else
                                        <a href="{{ $order->delivery->delivery_name == 'NovaPoshta' ? route('operator.order.novaPoshta.createTTN', $order->id) : '#' }}" class="p-2 hidden" onclick="{{ $order->delivery->delivery_name == 'UkrPoshta' ? 'getSender('. $order->id .')' : '' }}">
                                            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                        ...
                                        @if($order->delivery->delivery_name == 'UkrPoshta' || $order->delivery->delivery_name == 'Meest')
                                            <a href="{{ route('operator.order.addTtnToOrder', $order->id) }}" class="p-2" onclick="return isOperator({{ $order->operator_id ?? null }})">
                                                <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                                     width="20px" viewBox="0 0 401.994 401.994" style="enable-background:new 0 0 401.994 401.994;fill: green"
                                                     xml:space="preserve">
                                                <path d="M394,154.175c-5.331-5.33-11.806-7.994-19.417-7.994H255.811V27.406c0-7.611-2.666-14.084-7.994-19.414
...
<script>
    ...
    function isOperator(orderOperatorId) {
        if (!orderOperatorId) {
            alert('Спершу поміняйте статус');
            return false;
        }
        return true;
    }
