        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="max-width: 95rem">
            <div class="bg-transparent overflow-hidden shadow-sm sm:rounded-lg bg-white">
                <div class="p-6 text-gray-900">
                    <h1 class="text-2xl font-semibold mb-2 text-center">Топ товари</h1>
                    <div class="text-center mb-4">
                        <a href="{{ route('top-product.create') }}" class="bg-green-600 hover:bg-green-700 block text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out w-full border">Додати топ товар</a>
                    </div>
                    <table class="w-full mb-5">
                        ...
                        <tr class="text-center border-b-2 border-gray-700">
                            ...
                            <th class="p-2 text-lg">Кількість замовлень</th>
                        ...
                        @foreach ($topProducts as $product)
                            <tr class="text-center odd:bg-gray-200">
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">
                                    <img style="max-height: 80px;" alt="{{ $product->product->title }}" src="{{ asset('storage/'. $product->product->img_path) }}">
                                </td>
                                ...
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $product->product->price->retail }} UAH</td>
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $product->count_purchased }}</td>
                                <td class="px-6 py-4 text-right" style="vertical-align: middle; display: flex; gap: 10px;">
                                    <a href="{{ route('top-product.edit', $product->id) }}" title="Редагувати топ товар">
                                    ...
                                    <form action="{{ route('top-product.delete' , $product->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Видалити топ товар">
