        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="max-width: 95rem">
            <div class="bg-transparent overflow-hidden shadow-sm sm:rounded-lg bg-white">
                <div class="p-6 text-gray-900">
                    <h1 class="text-2xl font-semibold mb-2 text-center">Сертифікати</h1>
                    <div class="text-center mb-4">
                        <a href="{{ route('certificate.create') }}" class="bg-green-600 hover:bg-green-700 block text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out w-full border">Створити сертифікат</a>
                    </div>
                    <table class="w-full mb-5">
                        <thead>
                            <tr class="text-center border-b-2 border-gray-700">
                                <th class="p-2 text-lg">Сертифікат</th>
                        ...
                        @foreach ($certificates as $certificate)
                            <tr class="text-center odd:bg-gray-200">
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $certificate->title }}</td>
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $certificate->rate }} %</td>
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $certificate->quantity_now }}</td>
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $certificate->quantity }}</td>
                                <td class="px-6 py-4 text-right" style="vertical-align: middle; display: flex; gap: 10px;">
                                    <a href="{{ route('certificate.edit', $certificate->id) }}" title="Редагувати сертифікат">
                                    ...
                                    <form action="{{ route('promoCode.destroy', $certificate->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Видалити сертифікат">
