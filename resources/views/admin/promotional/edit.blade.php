        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="max-width: 95rem">
            <div class="bg-transparent overflow-hidden shadow-sm sm:rounded-lg bg-white">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('promotional.update', $product->id) }}" method="post">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-6 flex-1">
                            <div class="mb-4">
                                ...
                                <input type="number" min="0" max="80" class="w-full border rounded px-3 py-2" name="promotionalRate" id="promotionalRate" value="{{ $product->price->promotional_rate }}">
                            </div>
                            <div class="mb-4">
                                <label for="promotionalPrice" class="block mb-2 font-bold">Ціна зі знижкою %:</label>
                                <input type="text" class="w-full border rounded px-3 py-2" name="promotionalPrice" id="promotionalPrice" value="{{ $product->price->promotional_price }}" disabled>
                            </div>
                        </div>
...
<script>
    ...
    promotionalRate.addEventListener('change', function() {
        ...
        const originalPrice = {{ $product->price->pair }};
