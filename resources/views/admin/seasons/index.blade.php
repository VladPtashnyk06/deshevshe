        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="max-width: 95rem">
            <div class="bg-transparent overflow-hidden shadow-sm sm:rounded-lg bg-white">
                <div class="p-6 text-gray-900">
                    <h1 class="text-2xl font-semibold mb-2 text-center">Сезон</h1>
                    <table class="w-full mb-5">
                        ...
                        @foreach ($seasons as $season)
                            <tr class="text-center odd:bg-gray-200">
                                <td class="px-6 py-4" style="word-wrap:break-word; max-width: 15rem; vertical-align: top;">{{ $season->title }}</td>
                                <td class="px-6 py-4 text-right" style="vertical-align: middle; display: flex; gap: 10px;">
                                    <a href="{{ route('season.edit', $season->id) }}" title="Редагувати сезон">
                                    ...
                                    <form action="{{ route('season.destroy', $season->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" title="Видалити сезон">
