<x-site-layout>
    <main class="wrapper">
        <div class="d-flex flex-arrow">
            <a class="arrow-up arrow" href="#body">
                <svg fill="none" height="10" viewbox="0 0 18 10" width="18" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 9L9 0.999999L17 9" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path>
                </svg>
            </a>
            <a class="arrow-down arrow" href="#footer">
                <svg fill="none" height="10" viewbox="0 0 18 10" width="18" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 0.999999L9 9L17 1" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path>
                </svg>
            </a>
        </div>
        <p class="page-look d-flex">
            <a href="{{ route('site.index') }}">головна</a>
            <span>/</span>
            <span class="selected-page">контакти</span>
        </p>
        <div class="wrapper-contact">
            <div class="flex-between contact-page-flex">
                <div class="contact page-contact">
                    <h2>контактна інформація</h2>
                    <p class="d-flex">
                        <svg fill="none" height="17" viewbox="0 0 17 17" width="17" xmlns="http://www.w3.org/2000/svg">
                            <path d="M11.5 10.6012C9.50415 12.7018 4.41318 7.65653 6.41662 5.54795C7.63985 4.26052 6.25803 2.78923 5.49317 1.70778C4.0577 -0.321854 0.906427 2.48034 1.00213 4.26287C1.30394 9.88404 7.38462 16.5455 13.2729 15.9645C15.115 15.7827 17.2315 12.456 15.1186 11.2402C14.0621 10.6322 12.6118 9.43099 11.5 10.6012Z" stroke="#1E4284" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>
                        <a href="tel:+380000000000">+38 (000) 000 00 00</a>
                    </p>
                    <p class="d-flex">
                        <svg fill="none" height="10" viewbox="0 0 15 10" width="15" xmlns="http://www.w3.org/2000/svg">
                            <path clip-rule="evenodd" d="M1.875 0.9375H13.125C13.6428 0.9375 14.0625 1.35723 14.0625 1.875V8.125C14.0625 8.64277 13.6428 9.0625 13.125 9.0625H1.875C1.35723 9.0625 0.9375 8.64277 0.9375 8.125V1.875C0.9375 1.35723 1.35723 0.9375 1.875 0.9375ZM0 1.875C0 0.839466 0.839466 0 1.875 0H13.125C14.1605 0 15 0.839466 15 1.875V8.125C15 9.16053 14.1605 10 13.125 10H1.875C0.839466 10 0 9.16053 0 8.125V1.875ZM2.35405 2.60864C2.19114 2.45844 1.93354 2.46476 1.77868 2.62277C1.62382 2.78078 1.63034 3.03063 1.79324 3.18083L4.81459 5.96661C4.81997 5.97157 4.82548 5.97638 4.83113 5.98105L4.85188 5.99819C5.40924 6.45856 5.85647 6.82796 6.25018 7.07894C6.65807 7.33896 7.05015 7.5 7.5 7.5C7.94985 7.5 8.34193 7.33896 8.74982 7.07894C9.14353 6.82796 9.59076 6.45856 10.1481 5.99819L10.1481 5.99819L10.1689 5.98104C10.1745 5.97638 10.18 5.97157 10.1854 5.96661L13.2068 3.18083C13.3697 3.03063 13.3762 2.78078 13.2213 2.62277C13.0665 2.46476 12.8089 2.45844 12.646 2.60864L9.63268 5.38698C9.05424 5.86474 8.64749 6.19945 8.30277 6.41921C7.96565 6.63411 7.73022 6.71053 7.5 6.71053C7.26978 6.71053 7.03435 6.63411 6.69723 6.41921C6.35251 6.19945 5.94576 5.86474 5.36733 5.38698L2.35405 2.60864Z" fill="#1E4284" fill-rule="evenodd"></path>
                        </svg>
                        <a href="mailto:deshevshe.ukraine@gmail.com">deshevshe.ukraine@gmail.com</a>
                    </p>
                </div>
                <div class="social-contact page-contact">
                    <h2>ми у соцмережах</h2>
                    <div class="contact-social-page">
                        <div class="d-flex social-icon">
                            <p>
                                <a href="https://t.me/">
                                    <svg fill="none" height="20" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_213_16744)">
                                            <path d="M10 20C15.5228 20 20 15.5228 20 10C20 4.47715 15.5228 0 10 0C4.47715 0 0 4.47715 0 10C0 15.5228 4.47715 20 10 20Z" fill="url(#paint0_linear_213_16744)"></path>
                                            <path d="M6.7998 10.7L7.9998 14C7.9998 14 8.0998 14.3 8.2998 14.3C8.4998 14.3 10.7998 11.8 10.7998 11.8L13.3998 6.7L6.7998 9.9V10.7Z" fill="#C1DAEA"></path>
                                            <path d="M8.29966 11.6L8.09966 14C8.09966 14 7.99966 14.7 8.69966 14C9.39966 13.3 10.1997 12.7 10.1997 12.7L8.29966 11.6Z" fill="#9CC6D9"></path>
                                            <path d="M6.8001 10.8L4.4001 10C4.4001 10 4.1001 9.9 4.2001 9.6C4.2001 9.5 4.3001 9.5 4.4001 9.4C4.9001 9 14.4001 5.6 14.4001 5.6C14.4001 5.6 14.7001 5.5 14.8001 5.6C14.9001 5.6 14.9001 5.7 15.0001 5.8C15.0001 5.8 15.0001 5.9 15.0001 6C15.0001 6.1 15.0001 6.1 15.0001 6.2C14.9001 7.1 13.2001 14.1 13.2001 14.1C13.2001 14.1 13.1001 14.5 12.7001 14.5C12.6001 14.5 12.4001 14.5 12.2001 14.3C11.5001 13.7 9.0001 12 8.4001 11.6C8.4001 11.6 8.4001 11.6 8.4001 11.5V11.4C8.4001 11.4 12.8001 7.5 12.9001 7.1C12.9001 7.1 12.9001 7.1 12.8001 7.1C12.5001 7.2 7.5001 10.4 6.9001 10.7C6.9001 10.9 6.8001 10.8 6.8001 10.8Z" fill="white"></path>
                                        </g>
                                        <defs>
                                            <lineargradient gradientunits="userSpaceOnUse" id="paint0_linear_213_16744" x1="10" x2="10" y1="20" y2="0">
                                                <stop stop-color="#0093D4"></stop>
                                                <stop offset="1" stop-color="#00B0E5"></stop>
                                            </lineargradient>
                                            <clippath id="clip0_213_16744">
                                                <rect fill="white" height="20" width="20"></rect>
                                            </clippath>
                                        </defs>
                                    </svg>
                                </a>
                            </p>
                            <p>
                                <a href="whatsapp://send?phone=+12345678910"><svg fill="none" height="20" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_213_16752)">
                                            <path d="M10 20C15.5228 20 20 15.5228 20 10C20 4.47715 15.5228 0 10 0C4.47715 0 0 4.47715 0 10C0 15.5228 4.47715 20 10 20Z" fill="url(#paint0_linear_213_16752)"></path>
                                            <path clip-rule="evenodd" d="M14.8004 5.3C13.6004 4.1 12.0004 3.4 10.3004 3.4C6.80039 3.5 4.00039 6.3 4.00039 9.8C4.00039 10.9 4.30039 12 4.80039 13L3.90039 16.3L7.30039 15.4C8.20039 15.9 9.30039 16.2 10.3004 16.2C13.8004 16.2 16.6004 13.4 16.6004 9.9C16.7004 8.1 16.0004 6.5 14.8004 5.3ZM10.3004 15.1C9.40039 15.1 8.40039 14.8 7.60039 14.4L7.40039 14.3L5.40039 14.8L6.00039 12.8L5.90039 12.6C5.30039 11.8 5.00039 10.8 5.10039 9.8C5.10039 6.9 7.50039 4.5 10.4004 4.5C11.8004 4.5 13.1004 5 14.1004 6C15.1004 7 15.6004 8.3 15.6004 9.7C15.6004 12.7 13.2004 15.1 10.3004 15.1ZM13.2004 11.1C13.0004 11 12.3004 10.6 12.1004 10.6C12.0004 10.5 11.8004 10.5 11.7004 10.7C11.6004 10.9 11.3004 11.2 11.2004 11.3C11.1004 11.4 11.0004 11.4 10.9004 11.3C10.7004 11.2 10.2004 11.1 9.60039 10.5C9.10039 10.1 8.80039 9.6 8.70039 9.4C8.60039 9.2 8.70039 9.2 8.80039 9.1C8.90039 9.1 9.00039 9 9.10039 8.9C9.20039 8.8 9.20039 8.7 9.30039 8.6C9.40039 8.5 9.30039 8.4 9.30039 8.3C9.30039 8.2 8.90039 7.4 8.80039 7.1C8.60039 6.8 8.50039 6.9 8.40039 6.9C8.30039 6.9 8.20039 6.9 8.10039 6.9C8.00039 6.9 7.80039 6.9 7.70039 7.1C7.50039 7.2 7.10039 7.6 7.10039 8.4C7.10039 9.2 7.70039 9.9 7.70039 10C7.80039 10.1 8.80039 11.7 10.4004 12.4C10.8004 12.6 11.1004 12.7 11.3004 12.7C11.7004 12.8 12.0004 12.8 12.3004 12.8C12.6004 12.8 13.2004 12.4 13.4004 12C13.5004 11.6 13.5004 11.3 13.5004 11.2C13.5004 11.2 13.4004 11.2 13.2004 11.1Z" fill="white" fill-rule="evenodd"></path>
                                        </g>
                                        <defs>
                                            <lineargradient gradientunits="userSpaceOnUse" id="paint0_linear_213_16752" x1="10" x2="10" y1="18.547" y2="-1.453">
                                                <stop stop-color="#27CD44"></stop>
                                                <stop offset="1" stop-color="#5FFB7C"></stop>
                                            </lineargradient>
                                            <clippath id="clip0_213_16752">
                                                <rect fill="white" height="20" width="20"></rect>
                                            </clippath>
                                        </defs>
                                    </svg>
                                </a>
                            </p>
                            <p>
                                <a href=""><svg fill="none" height="20" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_213_16759)">
                                            <path d="M10 20C15.5228 20 20 15.5228 20 10C20 4.47715 15.5228 0 10 0C4.47715 0 0 4.47715 0 10C0 15.5228 4.47715 20 10 20Z" fill="black"></path>
                                            <path d="M7.29961 14C7.39961 14.1 7.49961 14.2 7.59961 14.3C7.89961 14.6 8.29961 14.7 8.69961 14.7C9.69961 14.8 10.5996 14 10.6996 13V12.9C10.6996 9.9 10.6996 6.9 10.6996 3.9C10.6996 3.7 10.6996 3.7 10.8996 3.7C11.3996 3.7 11.7996 3.7 12.2996 3.7H12.3996V3.8C12.5996 4.6 12.9996 5.2 13.6996 5.7L13.7996 5.8C13.7996 5.8 13.8996 5.8 13.8996 5.9C13.9996 6 14.0996 6.1 14.1996 6.1C14.5996 6.4 14.9996 6.6 15.3996 6.7H15.4996V6.8C15.4996 7.3 15.4996 7.9 15.4996 8.4C15.4996 8.5 15.4996 8.5 15.3996 8.5C14.9996 8.5 14.5996 8.4 14.2996 8.4C13.6996 8.3 13.0996 8 12.4996 7.6C12.4996 7.6 12.3996 7.5 12.3996 7.6V7.7C12.3996 9.2 12.3996 10.7 12.3996 12.2C12.3996 13.2 12.0996 14.2 11.3996 15C10.6996 15.8 9.79961 16.3 8.69961 16.5C7.89961 16.6 6.99961 16.5 6.29961 16.1C6.19961 16 6.09961 16 5.89961 15.9L5.79961 15.8C5.49961 15.6 5.39961 15.3 5.19961 15C4.89961 14.6 4.79961 14.1 4.69961 13.6C4.69961 13.3 4.59961 13 4.59961 12.7C4.69961 11.7 4.99961 10.9 5.59961 10.2C6.09961 9.6 6.79961 9.2 7.49961 8.9C7.79961 8.8 8.09961 8.7 8.49961 8.7C8.59961 8.7 8.59961 8.7 8.69961 8.7H8.79961V8.8C8.79961 9.3 8.79961 9.9 8.79961 10.4C8.79961 10.4 8.79961 10.4 8.79961 10.5C8.79961 10.6 8.79961 10.6 8.69961 10.6C8.59961 10.6 8.39961 10.5 8.29961 10.5C8.09961 10.5 7.79961 10.5 7.59961 10.6C6.89961 10.9 6.39961 11.4 6.29961 12.2C6.19961 13 6.49961 13.6 7.09961 14C7.19961 13.9 7.29961 14 7.29961 14Z" fill="white"></path>
                                            <path d="M5.90039 15.9C6.40039 16.2 7.00039 16.4 7.50039 16.5C7.80039 16.5 8.10039 16.6 8.30039 16.5C9.20039 16.5 10.1004 16.1 10.8004 15.5C11.5004 14.9 12.0004 14.2 12.2004 13.3C12.3004 13 12.3004 12.7 12.3004 12.5C12.3004 12.3 12.3004 12.2 12.3004 12C12.3004 10.6 12.3004 9.2 12.3004 7.8V7.7H12.4004C12.9004 8 13.4004 8.3 14.0004 8.5C14.3004 8.6 14.7004 8.6 15.0004 8.7C15.1004 8.7 15.3004 8.7 15.4004 8.7C15.5004 8.7 15.5004 8.7 15.5004 8.6C15.5004 8.5 15.5004 8.5 15.5004 8.4C15.5004 7.9 15.5004 7.4 15.5004 7V6.9C15.6004 6.9 15.8004 6.9 15.9004 7C16.0004 7 16.1004 7 16.1004 7C16.1004 7 16.2004 7 16.2004 7.1C16.2004 7.8 16.2004 8.5 16.2004 9.2C16.2004 9.3 16.2004 9.3 16.1004 9.3C15.9004 9.3 15.7004 9.3 15.5004 9.3C15.1004 9.2 14.6004 9.1 14.2004 9C13.8004 8.8 13.4004 8.6 13.1004 8.4C13.1004 8.4 13.1004 8.4 13.0004 8.3V8.4C13.0004 9.9 13.0004 11.4 13.0004 12.9C13.0004 13.7 12.8004 14.5 12.4004 15.1C11.9004 15.9 11.2004 16.5 10.2004 16.8C9.60039 17 9.10039 17.1 8.50039 17C7.60039 17 6.70039 16.6 5.90039 15.9Z" fill="#EC1655"></path>
                                            <path d="M12.5004 3.8C12.4004 3.8 12.4004 3.8 12.5004 3.8C11.9004 3.8 11.4004 3.8 10.9004 3.8C10.8004 3.8 10.8004 3.8 10.8004 3.9C10.8004 6.9 10.8004 9.9 10.8004 12.8C10.8004 13 10.8004 13.2 10.7004 13.4C10.5004 14.1 10.0004 14.6 9.30039 14.7C8.50039 14.9 7.90039 14.6 7.40039 14C7.50039 14.1 7.70039 14.1 7.90039 14.1C8.40039 14.2 8.90039 14.1 9.30039 13.9C9.80039 13.6 10.1004 13.1 10.2004 12.6C10.2004 12.5 10.2004 12.4 10.2004 12.3C10.2004 9.3 10.2004 6.3 10.2004 3.4C10.2004 3.3 10.2004 3.3 10.3004 3.3C11.0004 3.3 11.7004 3.3 12.4004 3.3C12.5004 3.3 12.5004 3.3 12.5004 3.4C12.4004 3.4 12.4004 3.6 12.5004 3.8Z" fill="#0CBAC6"></path>
                                            <path d="M8.8 8.70001C8.4 8.70001 8 8.70001 7.6 8.90001C7 9.10001 6.5 9.40001 6 9.80001C5.4 10.3 5 11 4.8 11.8C4.7 12.1 4.7 12.4 4.7 12.6C4.7 12.8 4.7 12.9 4.7 13.1C4.7 13.6 4.8 14 5 14.5C5.2 15 5.5 15.4 5.8 15.8C5.7 15.8 5.7 15.7 5.6 15.6C4.9 15.1 4.4 14.4 4.2 13.5C4.1 13.3 4 13 4 12.7C4 12.5 4 12.3 4 12C4 11.5 4.2 11 4.4 10.6C4.6 10.1 4.9 9.70001 5.3 9.30001C5.7 8.90001 6.2 8.50001 6.8 8.30001C7.2 8.20001 7.5 8.10001 7.9 8.10001C8.2 8.10001 8.5 8.10001 8.7 8.10001C8.7 8.10001 8.8 8.10001 8.8 8.20001C8.8 8.30001 8.8 8.50001 8.8 8.70001Z" fill="#0CBAC6"></path>
                                            <path d="M7.29997 14C7.09997 13.9 6.99997 13.8 6.89997 13.7C6.59997 13.4 6.39997 13 6.29997 12.5C6.19997 11.5 6.79997 10.7 7.59997 10.4C7.99997 10.3 8.29997 10.3 8.69997 10.4C8.69997 10.4 8.69997 10.4 8.79997 10.4V10.3C8.79997 9.80001 8.79997 9.20001 8.79997 8.70001V8.60001C8.99997 8.60001 9.19997 8.60001 9.39997 8.60001C9.39997 9.30001 9.39997 10 9.39997 10.8V10.9C9.29997 10.9 9.19997 10.9 9.09997 10.8C8.49997 10.7 7.99997 10.9 7.59997 11.2C7.19997 11.5 6.99997 11.9 6.89997 12.4C6.89997 13.1 6.99997 13.5 7.29997 14Z" fill="#EC1655"></path>
                                            <path d="M13.9004 5.9C13.7004 5.8 13.5004 5.6 13.3004 5.4C12.8004 4.9 12.6004 4.4 12.4004 3.8C12.6004 3.8 12.8004 3.8 13.0004 3.8C13.0004 4.3 13.2004 4.8 13.4004 5.3C13.6004 5.5 13.7004 5.7 13.9004 5.9Z" fill="#EC1655"></path>
                                            <path d="M15.6 6.9C15 6.8 14.6 6.5 14.1 6.2C14.1 6.1 14 6 14 6C14 6 14 6 14.1 6C14.4 6.2 14.8 6.3 15.2 6.4C15.3 6.4 15.5 6.4 15.7 6.4C15.8 6.4 15.8 6.4 15.8 6.5C15.6 6.6 15.6 6.7 15.6 6.9Z" fill="#0CBAC6"></path>
                                        </g>
                                        <defs>
                                            <clippath id="clip0_213_16759">
                                                <rect fill="white" height="20" width="20"></rect>
                                            </clippath>
                                        </defs>
                                    </svg>
                                </a>
                            </p>
                            <p>
                                <a href="">
                                    <svg fill="none" height="20" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_213_16771)">
                                            <path d="M10 0C4.5 0 0 4.5 0 10C0 15 3.7 19.1 8.4 19.9V12.9H5.9V10H8.4V7.8C8.4 5.3 9.9 3.9 12.2 3.9C13.3 3.9 14.4 4.1 14.4 4.1V6.6H13.1C11.9 6.6 11.5 7.4 11.5 8.2V10H14.3L13.9 12.9H11.6V19.9C16.3 19.1 20 15 20 10C20 4.5 15.5 0 10 0Z" fill="#1677F0"></path>
                                        </g>
                                        <defs>
                                            <clippath id="clip0_213_16771">
                                                <rect fill="white" height="20" width="20"></rect>
                                            </clippath>
                                        </defs>
                                    </svg>
                                </a>
                            </p>
                            <p>
                                <a href=""><svg fill="none" height="20" viewbox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_213_16773)">
                                            <path d="M10.0002 18.7C14.8603 18.7 18.8002 14.7601 18.8002 9.9C18.8002 5.03989 14.8603 1.1 10.0002 1.1C5.14009 1.1 1.2002 5.03989 1.2002 9.9C1.2002 14.7601 5.14009 18.7 10.0002 18.7Z" fill="white"></path>
                                            <path d="M10 20C4.5 20 0 15.5 0 10C0 4.5 4.5 0 10 0C15.5 0 20 4.5 20 10C20 15.5 15.5 20 10 20ZM13.4 5.9C13 5.9 12.7 6.2 12.7 6.6C12.7 7 13 7.3 13.4 7.3C13.8 7.3 14.1 7 14.1 6.6C14.1 6.2 13.8 5.9 13.4 5.9ZM10.1 6.9C8.4 6.9 7 8.3 7 10C7 11.7 8.4 13.1 10.1 13.1C11.8 13.1 13.2 11.7 13.2 10C13.2 8.3 11.8 6.9 10.1 6.9ZM10.1 12C9 12 8.1 11.1 8.1 10C8.1 8.9 9 8 10.1 8C11.2 8 12.1 8.9 12.1 10C12.1 11.1 11.2 12 10.1 12ZM16.4 7.4C16.4 5.3 14.7 3.6 12.6 3.6H7.6C5.5 3.6 3.8 5.3 3.8 7.4V12.4C3.8 14.5 5.5 16.2 7.6 16.2H12.6C14.7 16.2 16.4 14.5 16.4 12.4V7.4ZM15.2 12.4C15.2 13.8 14 15 12.6 15H7.6C6.1 15 5 13.9 5 12.4V7.4C5 6 6.1 4.8 7.6 4.8H12.6C14 4.8 15.2 6 15.2 7.4V12.4Z" fill="url(#paint0_linear_213_16773)"></path>
                                        </g>
                                        <defs>
                                            <lineargradient gradientunits="userSpaceOnUse" id="paint0_linear_213_16773" x1="1.92" x2="26.7298" y1="16.88" y2="-4.2452">
                                                <stop offset="0.0011048" stop-color="#FFE185"></stop>
                                                <stop offset="0.2094" stop-color="#FFBB36"></stop>
                                                <stop offset="0.3765" stop-color="#FF5176"></stop>
                                                <stop offset="0.5238" stop-color="#F63395"></stop>
                                                <stop offset="0.7394" stop-color="#A436D2"></stop>
                                                <stop offset="1" stop-color="#5F4EED"></stop>
                                            </lineargradient>
                                            <clippath id="clip0_213_16773">
                                                <rect fill="white" height="20" width="20"></rect>
                                            </clippath>
                                        </defs>
                                    </svg>
                                </a>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-contactPage">
                <form method="POST" action="/send-callback">
                    @csrf
                    <h1>Звʼязатися з нами</h1>
                    <div class="flex-between">
                        <p>
                            <label for="nameContact">Імʼя та прізвище*</label>
                            <input name="nameContact" placeholder="Імʼя та прізвище" type="text">
                        </p>
                        <p>
                            <label for="mailContact">E-mail*</label>
                            <input name="mailContact" placeholder="E-mail" type="mail">
                        </p>
                    </div>
                    <label for="textarea">Повідомлення</label>
                    <textarea id="textarea" name="textarea" placeholder="Ваше повідомлення"></textarea>
                    <button class="yellow-cta btn-card contact-cta" type="submit">надіслати</button>
                </form>
            </div>
        </div>
    </main>
</x-site-layout>
