        ...
        <div class="mx-auto sm:px-6 lg:px-8 max-w-7xl">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-3xl font-semibold mb-6 text-center">Вподобані продукти</h1>
                                    ...
                                    @foreach($product->getMedia($product->id) as $media)
                                        @if($media->getCustomProperty('main_image') === 1)
                                            <div class="relative">
                                                ...
                                                @endif
                                                <a href="{{ route('site.product.showOneProduct', $product->id) }}"><img src="{{ $media->getUrl() }}" alt="{{ $media->getCustomProperty('alt') }}" class="h-40 w-auto rounded-md object-cover mb-4" loading="lazy"></a>
                                            </div>
                                        @endif
                                    @endforeach
                                    <div class="absolute top-10 m-2 text-red-500" style="right: 16px">
                                        <a href="" class="heartLink" data-product-id="{{ $product->id }}">
