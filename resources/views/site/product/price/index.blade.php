...
@else
    @if(session()->get('currency') == 'UAH')
        <p class="text-lg mb-2">Стара ціна за шт - {{ $price->pair }} {{ session()->get('currency') }}.</p>
    ...
    @elseif(session()->get('currency') == 'EUR')
        <p class="text-lg mb-2">Стара ціна за шт - {{ round($price->pair / session()->get('currency_rate_eur'), 2)}} {{ session()->get('currency') }}.</p>
        <p class="text-lg mb-2">Акційна ца шт - {{ round($price->promotional_price / session()->get('currency_rate_eur'), 2)}} {{ session()->get('currency') }}.</p>
        <p class="text-lg mb-2">Знижка - {{ $price->promotional_rate }}%.</p>
    @elseif(session()->get('currency') == 'USD')
        <p class="text-lg mb-2">Стара ціна за шт - {{ round($price->pair / session()->get('currency_rate_usd'), 2)}} {{ session()->get('currency') }}.</p>
