    ...
    public function addToCart(CartRequest $request): RedirectResponse
    {
        ...
        if(session()->get('currency') == 'USD') {
            ...
            \Cart::add([
                ...
                'name' => $product->title,
                'price' => ($product->price->retail / $currency_rate_usd),
                'quantity' => 1,
                'attributes' => [
                    'product_id' => $product->id,
                    'code' => $product->code,
                    'color_id' => $request->post('color_id_popup'),
                    'color' => $productVariant->color->title,
                    'size_id' => $request->post('size_id_popup'),
                    'size' => $productVariant->size->title,
                    'product_quantity' => $productVariant->quantity,
                    'retail' => $product->price->retail / $currency_rate_usd,
                    'currency' => $currency,
            ...
            \Cart::add([
                'id' => $productVariant->id.'_p',
                'name' => $product->title,
                'price' => ($product->price->retail / $currency_rate_eur),
                'quantity' => 1,
                'attributes' => [
                    'product_id' => $product->id,
                    'code' => $product->code,
                    ...
                    'size' => $productVariant->size->title,
                    'product_quantity' => $productVariant->quantity,
                    'retail' => $product->price->retail / $currency_rate_eur,
                    'currency' => $currency,
                    'imageUrl' => $product->getFirstMediaUrl($product->id),
                ],
            ]);
